...
| 构建版本对速度的影响 | **可忽略**：b10840 与 b10889 实测统计不可区分（见 `results/engine-build-ab.txt`）。新构建的价值在自带 CUDA 13 运行时（免手工补 DLL）与打包规范 |

---

## 4. 实测资源账本 / Measured resource ledger

配置 `--n-cpu-moe 38 -c 262144 -ctk q8_0 -ctv q8_0`（**定稿**）、模型 AD-4.27bpw：

| 资源 Resource | 占用 Usage |
|---|---|
| 显存 VRAM | 模型 ~14.7 GiB（10 层专家 10.3 + 注意力常驻 4.4）+ KV 4.13 GiB + 计算缓冲 1.98 GiB + 循环态 0.11 GiB ≈ **20.9 / 24 GiB** |
| 内存 RAM（映射） | **≈ 71 GiB 映射工作集**（38 层专家 39.1 + 表 35.76），物理 64 GiB，稳态占用 94~96% |

配置 `--n-cpu-moe 42 -c 262144`（KV=f16 基线）：
...
| `--n-cpu-moe N` | 前 N 层的专家权重留在 CPU | **38（256K + q8 KV，定稿）** / 42（256K + f16 KV）/ 32（64K 速度档） |
| `-ctk / -ctv q8_0` | KV 缓存量化 | **开启**：省 4 GiB 显存换 4 层专家（+3.4~5%），精度实测无损 |
